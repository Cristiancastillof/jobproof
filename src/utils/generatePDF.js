...
const addPageFooter = (doc, pageNumber, totalPages, reportData) => {
  ...
  doc.text(`Generated by JobProof · ${reportNumber}`, PAGE.margin, PAGE.height - 8);
...
const addSummaryCard = (doc, x, y, width, title, value, accentColor = COLORS.blue) => {
...
const addPhotoGrid = async (doc, title, photos = [], y) => {
  ...
  y = addSectionTitle(doc, title, "Photographic evidence attached to this job report.", y);
  ...
  for (let index = 0; index < validPhotos.length; index += 1) {
    if (y + imageHeight + 13 > PAGE.height - PAGE.bottomSafe) {
      doc.addPage();
      y = 20;
      y = addSectionTitle(doc, title, "Continued photographic evidence.", y);
      x = leftX;
    }
    ...
    doc.setDrawColor(...COLORS.border);
    doc.setFillColor(...COLORS.white);
    doc.roundedRect(x, y, imageWidth, imageHeight, 3, 3, "FD");
    ...
    doc.text(String(index + 1).padStart(2, "0"), x + 9, y + 8, {
      align: "center",
    });
...
const addHeader = async (doc, reportData) => {
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...COLORS.white);
  doc.text(formatStatusLabel(status).toUpperCase(), PAGE.width - PAGE.margin - 29, 26.5, {
    align: "center",
  });
...
const addReportSummary = (doc, reportData, y) => {
  y = addSectionTitle(doc, "Report summary", "Quick overview of the job and current status.", y);
...
export const generatePDF = async (reportData = {}) => {
  ...
  if (hasPhotos) {
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(...COLORS.slate);
    doc.text("No before or after photos were attached to this report.", PAGE.margin + 5, y + 12);
